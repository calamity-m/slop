---
description: Use tuicr to open or attach to an interactive review session and process review comments
argument-hint: "[repo-directory-or-session] [review-intent]"
---

# tuicr Review Workflow

Adapted from https://github.com/agavra/tuicr/blob/main/skills/tuicr/SKILL.md
...
1. **User-led review of agent-generated changes**
   ...
   - Open or find the session, then retrieve the user's comments with
     `tuicr review comments` when they say comments are ready.
   - If explicitly waiting while the user reviews, poll the same command
...
2. **Agent review of an AI-generated patch**
   ...
   - Ask first when the workflow or session is ambiguous.
...
3. Choose the session:
   ...
   - For a PR review, pass the PR slug from the listing, such as
   ...
   - Until active-session discovery is formalized as a stable protocol, treat
     `"active": true` as a convenience signal. If slug resolution fails, ask
     the user for the slug or repo path used by the session.

The CLI works even if the agent is not running inside zellij, so do not require
zellij just to connect to an existing active session.
...
| Environment | Action |
|-------------|--------|
| `$ZELLIJ` is set | Run `~/.scripts/tuicr-zellij /path/to/repo` |
| `$TMUX` is set | Explain that this dotfiles repo currently provides only the zellij wrapper, then ask whether to proceed manually or use another available tmux wrapper |
...
If both `$TMUX` and `$ZELLIJ` are set, prefer zellij because this dotfiles repo
provides `~/.scripts/tuicr-zellij`.
...
If present, process those instructions. Otherwise prefer `tuicr review comments`;
it is the primary source of review feedback. If the wrapper mentions clipboard
export, ask the user to paste it only when the CLI comments are unavailable.
...
| No active session, zellij available | Start a new tuicr pane with `~/.scripts/tuicr-zellij /path/to/repo` |
| No active session, no zellij | Tell the user you are waiting for them to start `tuicr` |
