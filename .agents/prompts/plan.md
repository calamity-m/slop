...
Choose a short feature name and use the current local date. If a plan for this task already exists, continue it rather than replacing it. Build the plan incrementally as each stage produces new information.
...
- Record verified facts and open questions in the plan file. Reference concrete files and symbols where possible.
...
- Update the plan with the user's answers, confirmed assumptions, constraints, and explicit non-goals.
...
- Return to an earlier stage if review exposes missing research, ambiguity, or a flawed design.
...
- Logical, ordered implementation phases. Each phase must include:
...
Keep all implementation checkboxes unchecked. Keep the plan concise, but include everything an implementation agent needs to execute it without additional context or repeated investigation. Do not add speculative work.
