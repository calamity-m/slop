...
if [ ${BASH_VERSION:0:1} -lt 4 ]; then
   bind '"\C-f": " \C-b\C-k \C-u`_navi_widget_legacy`\e\C-e\C-a\C-y\C-h\C-e\e \C-y\ey\C-x\C-x\C-f"'
else
   bind -x '"\C-f": _navi_widget'
fi
...
navie() {
    ...
    if [ -n "$result" ]; then
        history -s "$result"
        echo "Command selected. Press [Up Arrow] to edit/run."
    fi
}
